...
if args.opt == "sgd":
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(opt,
                                               milestones=[args.num_epochs + 1],
                                               gamma=0.1)
elif args.opt == 'yf':
    opt = YFOptimizer(param_set, lr=args.sgd_lr, clip_thresh=None,
                      adapt_clip=False)
...
for ep in range(1, args.num_epochs + 1):
    if args.op_attack and (ep - 1) % args.op_iter == 0:
        net.train()
        opt.zero_grad()
        batch1, batch2, is_adv = opmaxmin(net, generator.decode, args.op_eps,
                                          num_images=50, \
                                          num_steps=500,
                                          embed_feats=args.op_embed_feats,
                                          z_lr=1e-4, lambda_lr=1e-4, ind=ep)
        if ch.sum(is_adv) > 0:
            loss = args.op_weight * ch.sum(
                ce(net(batch1), net(batch2), reduction=None) * is_adv) / ch.sum(
                is_adv)
            loss.backward()
            opt.step()
        else:
    ...
    for i, (images, labels) in enumerate(trainloader):
        ...
        if (i + 1) % 100 == 0:
            print(
                "Epoch {0} | Iteration {1} | Loss {2} | Adv Acc {3}".format(ep,
                                                                            i + 1,
                                                                            loss,
                                                                            acc))
        loss.backward()
    ...
    if args.validation_set:
        ...
        print("Adversarial Validation Accuracy: %f" % (
                val_num_correct / val_num_total).cpu().item())
        print("############################")

    if val_num_correct / val_num_total > best_adv_acc:
        ch.save(net.state_dict(),
                "results/%s_%s_%s_best" % (args.dataset, MODE, args.save_str))
        print("Saved model...")
    ...
    for (images, labels) in testloader:
        ...
        num_total += labels.shape[0]
    print("###### EPOCH {0} COMPLETE ######".format(ep))
    print("Adversarial Test Accuracy: %f" % (
            num_correct / num_total).cpu().item())
    print("############################")

    if ep % SAVE_ITERS == 0:
        ch.save(net.state_dict(),
                "results/%s_%s_%s_%d" % (args.dataset, MODE, args.save_str, ep))
        print("Saved model...")
